...
function calendarBeforeShowDayCreate(day) {
    return calendarBeforeShowDay(day, busy_at_create[property_hash], $(this));
}
 ...
 */
function propertyDashboardInit(event)
{
    ...
    {
        creation = true;
        var url = url_list.property_info[property_hash];
        ...
        $.post(
            ...
            function(response)
            {
                if (response.success == true) {
                    $('.property-title').html(response.data.property.title);

                    $('.blocking-create-guest-select').each(
                        function()
                        {
                            $(this).children().remove();

                            for (var i = 0; i <= response.data.property.max_people; i++) {
                                var option = $('<option>', {
                                    value: i,
                                    html: i
                                });
    ...
    function showInformationPopup(data)
    {
        ...
        busy_at_edit = busy_at_create[property_hash];
            ...
            case 'blocked':
                ...
                button_view.click(
                    function (event)
                    {
                        ...
                        handler_url = url_list.edit[property_hash];
    ...
    var handler_url = null;
    var collecting_form_id = null;
    ...
    {
        var first_day = $(day_selector + '.selected[data-property="' + property_hash + '"]').first();
        ...
        var last_day = $(day_selector + '.selected[data-property="' + property_hash + '"]').last();
        ...
        $(day_selector + '.selected:not(.arrival):not(.departure)[data-property="' + property_hash + '"]').removeClass('blocked')
            .removeClass('departure')
            .removeClass('arrival')
            .removeClass('busy')
            .attr('data-contract', '')
                ...
                function(response)
                {
                    if (response.success == true) {
                        ...
                        if (date_arrival != date_arrival_timestamp_global || date_departure != date_departure_timestamp_global) {
                            ...
                            var list = $(day_selector + '[data-property="' + property_hash + '"]').filter(
                                function() {
                            ...
                            $(day_selector).removeClass('selected');
                            list.addClass('selected');
                        }

                        var day_departure = $(day_selector + '.selected[data-timestamp="' + date_departure + '"][data-property="' + property_hash + '"]');

                        var day_arrival = $(day_selector + '.selected[data-timestamp="' + date_arrival + '"][data-property="' + property_hash + '"]');
                        ...
                        $(day_selector + '.selected:not(.arrival.departure)[data-property="' + property_hash + '"]').attr('data-contract', response.data.contract_id);
                        $(day_selector + '.selected:not(.arrival.departure)[data-property="' + property_hash + '"]').data('contract', response.data.contract_id);

                        $(day_selector + '.selected:not(.arrival)[data-property="' + property_hash + '"]').attr('data-contract-before', response.data.contract_id);
                        $(day_selector + '.selected:not(.arrival)[data-property="' + property_hash + '"]').data('contractBefore', response.data.contract_id);

                        $(day_selector + '.selected:not(.arrival.departure)[data-property="' + property_hash + '"]').addClass('blocked');

                        date_arrival = date_arrival_timestamp_global = null;
                        date_departure = date_departure_timestamp_global = null;

                        $(day_selector + '.selected[data-property="' + property_hash + '"]').addClass('busy');
                        property_hash = null;
            ...
            if ($(this).hasClass('busy')) {
                if (($(this).hasClass('departure') && !$(this).hasClass('arrival')) || (!$(this).hasClass('departure') && $(this).hasClass('arrival'))) {
                    ...
                    if (date_arrival !== null && date_departure !== null) {
                        $(day_selector + '.selected').removeClass('selected');
                    ...
                    if (date_arrival === null) {
                        ...
                        property_hash = $(this).data('property');
                    ...
                    if (property_hash == $(this).data('property')) {
                    ...
                    if (need_popup === true) {
                        ...
                        if (list.not('.departure').not('.arrival').hasClass('busy')) {
                            ...
                            handler_url = url_list.create[property_hash];
                    ...
                    $(day_selector + '[data-contract="' + contract_hash + '"][data-property="' + property_hash + '"]').addClass('selected');
                    $(day_selector + '[data-contract-before="' + contract_hash + '"][data-property="' + property_hash + '"]').addClass('selected');
                ...
                if (date_arrival !== null && date_departure !== null) {
                    ...
                    property_hash = date_arrival = date_departure = null;
                }
                ...
                if (date_arrival === null) {
                    ...
                    if ($(this).data('property') == property_hash) {
                        $(this).addClass('selected');
                    }
